function [V_D,V_RF,W_D,W_RF,MSE,n] = NewGEini(N_s,N_RF,H,Vn,W_opt)
 ...
 while(n<3 || (MSE(n-2)-MSE(n-1))>1e-4 &&n<=10)
     ...
     Vn1 = tw*Vn;
     [V_RF,V_D] = NewTransGE (N_RF,H_u,Vn1,V_RF);
     tv = trace(V_RF*(V_D)*V_D'*V_RF');     
     %%UEside
     H_d = H*V_RF*V_D;
     Vn2 = tv*Vn;
     [W_RF,W_D] = NewTransGE (N_RF,H_d,Vn2,W_RF); %the same formulation
